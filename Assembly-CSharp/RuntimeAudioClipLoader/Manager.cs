	...
	public class Manager : MonoBehaviour
	{
		private class AudioInstance
		{
			...
			public int channels
			{
				get
				{
					return this.reader.WaveFormat.get_Channels();
				}
			}
			...
			public int sampleRate
			{
				get
				{
					return this.reader.WaveFormat.get_SampleRate();
				}
			}
		...
		public static AudioClip Load(Stream dataStream, AudioFormat audioFormat, string unityAudioClipName, bool doStream = false, bool loadInBackground = true, bool diposeDataStreamIfNotNeeded = true)
		{
			...
			try
			{
				...
				{
					reader = reader,
					samplesCount = (int)(reader.Length / (long)(reader.WaveFormat.get_BitsPerSample() / 8))
				};
				if (doStream)
				{
