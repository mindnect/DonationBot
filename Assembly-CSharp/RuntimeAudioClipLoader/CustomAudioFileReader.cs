...
namespace RuntimeAudioClipLoader
{
	internal class CustomAudioFileReader : WaveStream, ISampleProvider
	{
		...
		public override WaveFormat WaveFormat
		{
			get
			{
				return this.sampleChannel.get_WaveFormat();
			}
		}
		...
		public float Volume
		{
			get
			{
				return this.sampleChannel.get_Volume();
			}
			set
			{
				this.sampleChannel.set_Volume(value);
			}
		}
		...
		public CustomAudioFileReader(Stream stream, AudioFormat format)
		{
			...
			this.sourceBytesPerSample = this.readerStream.get_WaveFormat().get_BitsPerSample() / 8 * this.readerStream.get_WaveFormat().get_Channels();
			this.sampleChannel = new SampleChannel(this.readerStream, false);
			this.destBytesPerSample = 4 * this.sampleChannel.get_WaveFormat().get_Channels();
		...
		private void CreateReaderStream(Stream stream, AudioFormat format)
		{
			if (format == AudioFormat.wav)
			{
				...
				if (this.readerStream.get_WaveFormat().get_Encoding() != 1 && this.readerStream.get_WaveFormat().get_Encoding() != 3)
				{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			int count2 = count / 4;
			int num = this.Read(waveBuffer.get_FloatBuffer(), offset / 4, count2);
			return num * 4;
		}
