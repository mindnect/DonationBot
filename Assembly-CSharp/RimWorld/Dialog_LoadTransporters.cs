...
namespace RimWorld
{
	public class Dialog_LoadTransporters : Window
	{
		...
		private float MassUsage
		{
			get
			{
				return CollectionsMassCalculator.MassUsageTransferables(this.transferables, false, true, false);
			}
		}
		...
		private bool TryAccept()
		{
			...
			for (int i = 0; i < this.transferables.Count; i++)
			{
				...
				{
					this.transporters[i % this.transporters.Count].AddToTheToLoadList(this.transferables[i], this.transferables[i].countToTransfer);
				}
			}
