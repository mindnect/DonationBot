	...
	public class Dialog_FormCaravan : Window
	{
		...
		private float MassUsage
		{
			get
			{
				...
				return CollectionsMassCalculator.MassUsageTransferables(this.transferables, false, false, autoStripCorpses);
			}
		}
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			if (this.tab != Dialog_FormCaravan.Tab.Config)
			{
				...
				CaravanUIUtility.DrawDaysWorthOfFoodInfo(new Rect(rect3.x, rect3.y + 22f, rect3.width, rect3.height), DaysWorthOfFoodCalculator.ApproxDaysWorthOfFood(this.transferables), true);
			}
		...
		private void DoBottomButtons(Rect rect)
		{
			...
			if (Widgets.ButtonText(rect2, "AcceptButton".Translate(), true, false, true))
			{
				...
				{
					float num = DaysWorthOfFoodCalculator.ApproxDaysWorthOfFood(this.transferables);
					if (num < 5f)
					{
						if (this.CheckForErrors(TransferableUtility.GetPawnsFromTransferables(this.transferables)))
						{
							string text = (num >= 0.1f) ? "DaysWorthOfFoodWarningDialog".Translate(new object[]
							{
								num.ToString("0.#")
			...
			{
				float num2 = 200f;
				float num3 = this.BottomButtonSize.y / 2f;
				Rect rect5 = new Rect(rect.width - num2, rect.height - 55f, num2, num3);
				if (Widgets.ButtonText(rect5, "Dev: Send instantly", true, false, true) && this.DebugTryFormCaravanInstantly())
				{
					SoundDefOf.TickHigh.PlayOneShotOnCamera();
					this.Close(false);
				}
				Rect rect6 = new Rect(rect.width - num2, rect.height - 55f + num3, num2, num3);
