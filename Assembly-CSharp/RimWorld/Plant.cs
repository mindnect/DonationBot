...
namespace RimWorld
{
	...
	public class Plant : Thing
	{
		...
		public virtual void MakeLeafless()
		{
			...
			{
				if (this.IsCrop && MessagesRepeatAvoider.MessageShowAllowed("MessagePlantDiedOfCold-" + this.def.defName, 15f))
				{
			...
			if (GenPlant.GrowthSeasonNow(base.Position, base.Map))
			{
				...
				if (this.def.plant.LimitedLifespan)
				{
					...
					if (this.Dying)
					{
						...
						if (base.Destroyed)
						{
							if (isCrop && this.def.plant.Harvestable && MessagesRepeatAvoider.MessageShowAllowed("MessagePlantDiedOfRot-" + this.def.defName, 15f))
							{
