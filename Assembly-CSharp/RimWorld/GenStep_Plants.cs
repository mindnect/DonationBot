...
namespace RimWorld
{
	public class GenStep_Plants : GenStep
	{
		...
		{
			GenStep_Plants.<Generate>c__AnonStorey2C0 <Generate>c__AnonStorey2C = new GenStep_Plants.<Generate>c__AnonStorey2C0();
			<Generate>c__AnonStorey2C.map = map;
			<Generate>c__AnonStorey2C.map.regionAndRoomUpdater.Enabled = false;
			List<ThingDef> list = <Generate>c__AnonStorey2C.map.Biome.AllWildPlants.ToList<ThingDef>();
			for (int i = 0; i < list.Count; i++)
			{
				GenStep_Plants.numExtant.Add(list[i], 0);
			}
			GenStep_Plants.desiredProportions = GenPlant.CalculateDesiredPlantProportions(<Generate>c__AnonStorey2C.map.Biome);
			float num = <Generate>c__AnonStorey2C.map.Biome.plantDensity * <Generate>c__AnonStorey2C.map.mapConditionManager.AggregatePlantDensityFactor();
			foreach (IntVec3 c in <Generate>c__AnonStorey2C.map.AllCells.InRandomOrder(null))
			{
				if (c.GetEdifice(<Generate>c__AnonStorey2C.map) == null && c.GetCover(<Generate>c__AnonStorey2C.map) == null)
				{
					float num2 = <Generate>c__AnonStorey2C.map.fertilityGrid.FertilityAt(c);
					float num3 = num2 * num;
					if (Rand.Value < num3)
					{
						IEnumerable<ThingDef> source = from def in list
						where def.CanEverPlantAt(c, <Generate>c__AnonStorey2C.map)
						select def;
						if (source.Any<ThingDef>())
						{
							ThingDef thingDef = source.RandomElementByWeight((ThingDef x) => GenStep_Plants.PlantChoiceWeight(x, <Generate>c__AnonStorey2C.map));
							...
							for (int j = 0; j < randomInRange; j++)
							{
								...
								else if (!GenPlantReproduction.TryFindReproductionDestination(c, thingDef, SeedTargFindMode.MapGenCluster, <Generate>c__AnonStorey2C.map, out c2))
								{
									break;
								}
								...
								GenSpawn.Spawn(plant, c2, <Generate>c__AnonStorey2C.map);
			...
			<Generate>c__AnonStorey2C.map.regionAndRoomUpdater.Enabled = true;
		}
