...
namespace RimWorld
{
	public static class ApparelUtility
	{
		...
		{
			ApparelUtility.<HasPartsToWear>c__AnonStorey2EF <HasPartsToWear>c__AnonStorey2EF = new ApparelUtility.<HasPartsToWear>c__AnonStorey2EF();
			...
			<HasPartsToWear>c__AnonStorey2EF.groups = apparel.apparel.bodyPartGroups;
			int i;
			for (i = 0; i < <HasPartsToWear>c__AnonStorey2EF.groups.Count; i++)
			{
				if (notMissingParts.Any((BodyPartRecord x) => x.IsInGroup(<HasPartsToWear>c__AnonStorey2EF.groups[i])))
				{
					return true;
				}
			}
