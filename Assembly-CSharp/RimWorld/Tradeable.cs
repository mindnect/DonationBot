...
namespace RimWorld
{
	public class Tradeable : ITransferable
	{
		...
		private const float PriceFactorBuy_Global = 1.15f;

		private const float PriceFactorSell_Global = 0.85f;
		...
		private void InitPriceDataIfNeeded()
		{
			...
			this.pricePlayerBuy = this.BaseMarketValue * 1.15f * this.priceFactorBuy_TraderPriceType * (1f + Find.Storyteller.difficulty.tradePriceFactorLoss);
			...
			this.pricePlayerSell = this.BaseMarketValue * 0.85f * this.priceFactorSell_TraderPriceType * this.priceFactorSell_ItemSellPriceFactor * (1f - Find.Storyteller.difficulty.tradePriceFactorLoss);
