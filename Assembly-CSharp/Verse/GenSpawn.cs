...
namespace Verse
{
	public static class GenSpawn
	{
		...
		public static Thing Spawn(Thing newThing, IntVec3 loc, Map map, Rot4 rot)
		{
			...
			GenSpawn.WipeExistingThings(loc, rot, newThing.def, map, false);
		...
		public static void WipeExistingThings(IntVec3 thingPos, Rot4 thingRot, BuildableDef thingDef, Map map, bool reclaimResources)
		{
			GenSpawn.WipeExistingThings(thingPos, thingRot, thingDef, map, reclaimResources, null);
		}

		public static void WipeExistingThings(IntVec3 thingPos, Rot4 thingRot, BuildableDef thingDef, Map map, bool reclaimResources, CellRect? leavingsRect)
		{
				...
				foreach (Thing current2 in map.thingGrid.ThingsAt(current).ToList<Thing>())
				{
					if (GenSpawn.SpawningWipes(thingDef, current2.def))
					{
						if (reclaimResources && current2 is Building)
						{
							...
							GenLeaving.DoLeavingsFor(current2, map, DestroyMode.Deconstruct, leavingsRect2);
						}
