...
namespace Verse
{
	public sealed class MapPawns
	{
		...
		public IEnumerable<Pawn> AllPawns
		{
			get
			{
				MapPawns.<>c__Iterator1D5 <>c__Iterator1D = new MapPawns.<>c__Iterator1D5();
				<>c__Iterator1D.<>f__this = this;
				MapPawns.<>c__Iterator1D5 expr_0E = <>c__Iterator1D;
				expr_0E.$PC = -2;
				return expr_0E;
			}
		}
		...
		public bool AnyColonistTameAnimalOrPrisonerOfColony
		{
			get
			{
				...
				foreach (Pawn current in this.AllPawnsUnspawned)
				{
					if (current.Faction == ofPlayer || current.HostFaction == ofPlayer)
					{
						return true;
					}
				}
