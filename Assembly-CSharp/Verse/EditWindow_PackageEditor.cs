	...
	public class EditWindow_PackageEditor<TNewDef> : EditWindow where TNewDef : Def, new()
	{
		...
		public override void DoWindowContents(Rect selectorInner)
		{
			...
			else
			{
				...
				{
					EditWindow_PackageEditor<TNewDef>.<DoWindowContents>c__AnonStorey513 <DoWindowContents>c__AnonStorey = new EditWindow_PackageEditor<TNewDef>.<DoWindowContents>c__AnonStorey513();
					<DoWindowContents>c__AnonStorey.<>f__this = this;
					<DoWindowContents>c__AnonStorey.deletingDef = null;
					foreach (Def def in this.curPackage)
					{
						if (listing_Standard.SelectableDef(def.defName, false, delegate
						{
							<DoWindowContents>c__AnonStorey.deletingDef = def;
						}))
						{
							bool flag = false;
					...
					if (<DoWindowContents>c__AnonStorey.deletingDef != null)
					{
						Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation("Really delete Def " + <DoWindowContents>c__AnonStorey.deletingDef.defName + "?", delegate
						{
							<DoWindowContents>c__AnonStorey.<>f__this.curPackage.RemoveDef(<DoWindowContents>c__AnonStorey.deletingDef);
						}, true, null));
					}
				}
