	...
	public static class Workshop
	{
		...
		internal static void Upload(WorkshopUploadable item)
		{
			...
			if (Workshop.uploadingHook.PublishedFileId != PublishedFileId_t.Invalid)
			{
				...
				SteamAPICall_t steamAPICall_t = SteamUGC.SubmitItemUpdate(Workshop.curUpdateHandle, "[Auto-generated text]: Update on " + DateTime.Now.ToString() + ".");
				...
				Workshop.submitResult.Set(steamAPICall_t, null);
			}
			else
			{
				if (Prefs.LogVerbose)
				...
				SteamAPICall_t steamAPICall_t2 = SteamUGC.CreateItem(SteamUtils.GetAppID(), 0);
				...
				Workshop.createResult.Set(steamAPICall_t2, null);
			}
		...
		internal static void RequestItemsDetails(PublishedFileId_t[] publishedFileIds)
		{
			...
			SteamAPICall_t steamAPICall_t = SteamUGC.SendQueryUGCRequest(Workshop.detailsQueryHandle);
			...
			Workshop.requestDetailsResult.Set(steamAPICall_t, null);
		}
		...
		{
			if (IOFailure || result.m_eResult != 1)
			{
		...
		private static void OnGotItemDetails(SteamUGCRequestUGCDetailsResult_t result, bool IOFailure)
		{
			...
			for (int i = 0; i < Workshop.detailsQueryCount; i++)
			{
				...
				SteamUGC.GetQueryUGCResult(Workshop.detailsQueryHandle, (uint)i, ref steamUGCDetails_t);
				if (steamUGCDetails_t.m_eResult != 1)
				{
		...
		public static void GetUpdateStatus(out EItemUpdateStatus updateStatus, out float progPercent)
		{
			ulong num;
			ulong num2;
			updateStatus = SteamUGC.GetItemUpdateProgress(Workshop.curUpdateHandle, ref num, ref num2);
		...
		{
			Workshop.<AllSubscribedItems>c__Iterator225 <AllSubscribedItems>c__Iterator = new Workshop.<AllSubscribedItems>c__Iterator225();
			Workshop.<AllSubscribedItems>c__Iterator225 expr_07 = <AllSubscribedItems>c__Iterator;
			expr_07.$PC = -2;
			return expr_07;
		}
		...
		private static string ItemStatusString(PublishedFileId_t pfid)
		{
			...
			string str;
			uint num2;
			if (SteamUGC.GetItemInstallInfo(pfid, ref num, ref str, 257u, ref num2))
			{
				text += "\n      installed";
				text = text + "\n      folder=" + str;
