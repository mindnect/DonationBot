...
namespace RimWorld.Planet
{
	public static class DaysWorthOfFoodCalculator
	{
				...
				for (int k = 0; k < pawns.Count; k++)
				{
					...
					if (DaysWorthOfFoodCalculator.tmpAnyFoodLeftIngestibleByPawn[k])
					{
						if (!pawn.RaceProps.Eats(FoodTypeFlags.Plant))
						{
							do
							{
								int num2 = DaysWorthOfFoodCalculator.BestFoodIndexFor(pawns[k], DaysWorthOfFoodCalculator.tmpFood);
								if (num2 < 0)
								{
									goto Block_9;
								}
		...
		private static int BestFoodIndexFor(Pawn pawn, List<ThingCount> food)
		{
			int num = -1;
			float num2 = 0f;
			for (int i = 0; i < food.Count; i++)
			{
				if (food[i].Count > 0)
				{
					...
					if (thingDef.IsNutritionGivingIngestible && pawn.RaceProps.CanEverEat(thingDef))
					{
