...
namespace RimWorld.Planet
{
	public class Dialog_SplitCaravan : Window
	{
		...
		private float SourceMassCapacity
		{
			get
			{
				return CollectionsMassCalculator.CapacityLeftAfterTransfer(this.transferables);
			}
		}
		...
		private float SourceMassUsage
		{
			get
			{
				return CollectionsMassCalculator.MassUsageLeftAfterTransfer(this.transferables, true, false, false);
			}
		}
		...
		private float DestMassCapacity
		{
			get
			{
				return CollectionsMassCalculator.CapacityTransferables(this.transferables);
			}
		}
		...
		private float DestMassUsage
		{
			get
			{
				return CollectionsMassCalculator.MassUsageTransferables(this.transferables, true, false, false);
			}
		}
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			Dialog_SplitCaravan.tabsList.Add(new TabRecord("CaravanPawnsTab".Translate(), delegate
			{
				this.tab = Dialog_SplitCaravan.Tab.Pawns;
			}, this.tab == Dialog_SplitCaravan.Tab.Pawns));
			Dialog_SplitCaravan.tabsList.Add(new TabRecord("CaravanItemsTab".Translate(), delegate
			{
			...
			if (tab != Dialog_SplitCaravan.Tab.Pawns)
			{
				...
				{
					this.itemsTransfer.OnGUI(inRect2);
				}
			}
			else
			{
				this.pawnsTransfer.OnGUI(inRect2);
			}
			GUI.EndGroup();
		}
		...
		private void DrawMassAndFoodInfo(Rect rect)
		{
			...
			CaravanUIUtility.DrawDaysWorthOfFoodInfo(new Rect(rect.x, rect.y + 22f, rect.width, rect.height), DaysWorthOfFoodCalculator.ApproxDaysWorthOfFoodLeftAfterTransfer(this.transferables), false);
			...
			CaravanUIUtility.DrawDaysWorthOfFoodInfo(new Rect(rect.x, rect.y + 22f, rect.width, rect.height), DaysWorthOfFoodCalculator.ApproxDaysWorthOfFood(this.transferables), true);
		}
