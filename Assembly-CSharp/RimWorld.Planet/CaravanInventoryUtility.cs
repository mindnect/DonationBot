...
namespace RimWorld.Planet
{
	public static class CaravanInventoryUtility
	{
		...
		public static bool TryGetBestFood(Caravan caravan, Pawn forPawn, out Thing food, out Pawn owner)
		{
			...
			bool flag = forPawn.IsTeetotaler();
			Thing thing = null;
			float num = 0f;
			for (int i = 0; i < list.Count; i++)
			{
				Thing thing2 = list[i];
				if (thing2.IngestibleNow && thing2.def.IsNutritionGivingIngestible && forPawn.RaceProps.CanEverEat(thing2))
				{
					if (!flag || !thing2.def.IsDrug)
					{
