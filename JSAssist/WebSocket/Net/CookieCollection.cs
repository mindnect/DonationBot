...
namespace WebSocket.Net
{
	...
	public class CookieCollection : ICollection, IEnumerable
	{
		...
		private static int compareCookieWithinSorted(Cookie x, Cookie y)
		{
			...
			int num1 = x.Name.CompareTo(y.Name);
			num = num1;
			if (num1 != 0)
			{
				return num;
		...
		internal static CookieCollection Parse(string value, bool response)
		{
			if (!response)
			{
				return CookieCollection.parseRequest(value);
			}
			return CookieCollection.parseResponse(value);
		}
			...
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				...
				if (str1.Length != 0)
				{
					...
					else if (str1.StartsWith("expires", StringComparison.InvariantCultureIgnoreCase))
					{
						...
						if (localTime != null && localTime.Expires == DateTime.MinValue)
						{
		...
		private int searchCookie(Cookie cookie)
		{
			...
			{
				Cookie item = this._list[i];
				if (item.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase) && item.Path.Equals(path, StringComparison.InvariantCulture) && item.Domain.Equals(domain, StringComparison.InvariantCultureIgnoreCase) && item.Version == version)
				{
					return i;
				}
			}
		...
		{
			return (new List<string>(Ext.SplitHeaderValue(value, new char[] { ',', ';' }))).ToArray();
		}
	}
}