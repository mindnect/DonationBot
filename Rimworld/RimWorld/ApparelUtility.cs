...
namespace RimWorld
{
	public static class ApparelUtility
	{
		...
		{
			ApparelUtility.<HasPartsToWear>c__AnonStorey2F0 <HasPartsToWear>c__AnonStorey2F = new ApparelUtility.<HasPartsToWear>c__AnonStorey2F0();
			...
			<HasPartsToWear>c__AnonStorey2F.groups = apparel.apparel.bodyPartGroups;
			int i;
			for (i = 0; i < <HasPartsToWear>c__AnonStorey2F.groups.Count; i++)
			{
				if (notMissingParts.Any((BodyPartRecord x) => x.IsInGroup(<HasPartsToWear>c__AnonStorey2F.groups[i])))
				{
					return true;
				}
			}
