...
namespace RimWorld
{
	public abstract class WorkGiver_ConstructDeliverResources : WorkGiver_Scanner
	{
		...
		protected Job ResourceDeliverJobFor(Pawn pawn, IConstructible c)
		{
			...
			<ResourceDeliverJobFor>c__AnonStorey27C.pawn = pawn;
			...
			if (blueprint_Install == null)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (!<ResourceDeliverJobFor>c__AnonStorey27C.pawn.Map.itemAvailability.ThingsAvailableAnywhere(<ResourceDeliverJobFor>c__AnonStorey27D.need, <ResourceDeliverJobFor>c__AnonStorey27C.pawn))
					{
						flag = true;
						break;
					}
					...
					arg_158_0.foundRes = GenClosest.ClosestThingReachable(<ResourceDeliverJobFor>c__AnonStorey27C.pawn.Position, <ResourceDeliverJobFor>c__AnonStorey27C.pawn.Map, ThingRequest.ForDef(<ResourceDeliverJobFor>c__AnonStorey27D.need.thingDef), PathEndMode.ClosestTouch, TraverseParms.For(<ResourceDeliverJobFor>c__AnonStorey27C.pawn, Danger.Deadly, TraverseMode.ByPawn, false), 9999f, validator, null, -1, false);
					if (<ResourceDeliverJobFor>c__AnonStorey27D.foundRes != null)
					{
						int num = Mathf.Min(<ResourceDeliverJobFor>c__AnonStorey27D.foundRes.def.stackLimit, <ResourceDeliverJobFor>c__AnonStorey27C.pawn.carryTracker.MaxStackSpaceEver(<ResourceDeliverJobFor>c__AnonStorey27D.foundRes.def));
						int num2 = 0;
						...
						if (num2 < num)
						{
							foreach (Thing current in GenRadial.RadialDistinctThingsAround(<ResourceDeliverJobFor>c__AnonStorey27D.foundRes.Position, <ResourceDeliverJobFor>c__AnonStorey27D.foundRes.Map, 5f, false))
							{
								...
								{
									if (GenAI.CanUseItemForWork(<ResourceDeliverJobFor>c__AnonStorey27C.pawn, current))
									{
						...
						foreach (Thing current2 in GenRadial.RadialDistinctThingsAround(thing.Position, thing.Map, 8f, false))
						{
							...
							if (constructible != null)
							{
								if (constructible != c)
								{
									...
									{
										if (current2.Faction == <ResourceDeliverJobFor>c__AnonStorey27C.pawn.Faction)
										{
											if (!current2.IsForbidden(<ResourceDeliverJobFor>c__AnonStorey27C.pawn))
											{
												if (current2 != c && !hashSet.Contains(current2))
												{
													if (GenConstruct.CanConstruct(current2, <ResourceDeliverJobFor>c__AnonStorey27C.pawn))
													{
			...
			if (blueprint_Install.MiniToInstallOrBuildingToReinstall.IsForbidden(<ResourceDeliverJobFor>c__AnonStorey27C.pawn) || !<ResourceDeliverJobFor>c__AnonStorey27C.pawn.CanReserveAndReach(blueprint_Install.MiniToInstallOrBuildingToReinstall, PathEndMode.OnCell, <ResourceDeliverJobFor>c__AnonStorey27C.pawn.NormalMaxDanger(), 1))
			{
				return null;
			}
